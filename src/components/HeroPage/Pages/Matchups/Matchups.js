...
export default function Matchups({ heroData, initRole, heroMatchups }) {
    ...
    return(
        <div className='space-y-4 bg-slate-900 py-4 px-6 mx-auto rounded-lg border border-slate-800'>
            <div className="sm:flex items-center gap-2.5 px-2 sm:px-0 justify-between space-y-4 sm:space-y-0">
                ...
                <div className="sm:flex items-center justify-center space-x-2 space-y-4 sm:w-1/3">
                    ...
                    <div className='flex sm:hidden justify-center'>
                        <button 
                            className={`text-lg rounded-l-full py-1 px-2 text-center w-24 border border-slate-700 ${wORa == 'Against' ? 'bg-cyan-300 text-black font-bold' : ''}`}
                            onClick={() => setWORA('Against')}
                        >
                            Against
                        </button>
                        <button 
                            className={`text-lg rounded-r-full py-1 px-2 text-center w-24 border border-slate-700 ${wORa == 'With' ? 'bg-cyan-300 text-black font-bold' : ''}`}
