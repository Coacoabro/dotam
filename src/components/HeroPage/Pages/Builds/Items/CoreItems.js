...
export default function CoreItems({core, isCarry}) {
    ...
    if(core){
        ...
        return(
            <>
                <div 
                    className={`flex items-center gap-3 sm:gap-4 px-4 pb-2 pt-4 ${!isCarry ? "justify-center" : ""}`}
                ...
                >
                    ...
                    <div>
                        <div className='flex items-center gap-1'>
                            <h1 className='text-base sm:text-[16px]/[24px]'>{wr}%</h1>
                            <h2 className='text-xs sm:text-[13px]/[16px] opacity-50'>WR</h2>
                        </div>
                        <div className='text-2xs sm:text-[13px]/[16px] opacity-50 whitespace-nowrap truncate'>{dispMatches} Matches</div>
                    </div>
                </div>
                {showPermutations ? (
