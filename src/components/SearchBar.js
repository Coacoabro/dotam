...
export default function SearchBar( {scrollY, topBar} ) {
  ...
  return (
    <div ref={searchBarRef} className={`${scrollY == 0 ? 'w-[300px] sm:w-[800px] lg:w-[1200px]' : topBar ? 'w-[300px]' : 'w-[300px] sm:w-700px lg:w-[800px]'} transition-all duration-500 ease-in-out mx-auto shadow-sm rounded-[36px] overflow-hidden border border-slate-700 flex items-center`}>
      ...
      <button 
        className={`${topBar ? 'w-[70px] text-sm' : 'w-[80px] text-xs sm:text-md sm:w-[94px]'} h-[44px] sm:px-5 py-[9px] bg-gray-900 z-20 border-l border-slate-700 flex justify-center items-center text-slate-200 font-medium tracking-wide`}
        onClick={() => goToHero()}
      >
        Search
      </button>
