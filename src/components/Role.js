        ...
        <div className="flex h-8 gap-1 items-center">
            {router.asPath.includes('tier-list') || router.pathname == '/' || router.pathname == '/heroes' ? json.map((role) => (
                <button 
                    ...
                    className={`
                        text-sm font-bold items-center flex justify-center 
                        w-10 sm:w-12 h-7 sm:h-8 py-1 px-2 rounded-lg 
                        hover:bg-slate-600 border-white/15 border 
                        ${currRole == role.role ? 'bg-cyan-300 text-black' : 'text-white'}
                    `}
                >
                    <div className={`${hovered == role.role && hovered != "" ? "" : "hidden"} absolute -mt-20 bg-slate-950 border border-white py-1 px-2 rounded-lg text-white`}>
                        {role.name}
                    </div>
            ...
            json.filter((r) => r.role !== "").map((role) => (
                <button 
                    ...
                    className={`text-sm font-bold items-center flex justify-center w-12 h-8 py-1 px-2 rounded-lg hover:bg-slate-600 
                        ${currRole == role.role ? 'bg-cyan-300 text-black' : 'text-white border bg-slate-950'} 
                        ${initRole == role.role ? 'border-cyan-300 border-2' : 'border-white/15 '}
                    `}
                >
