    ...
    return(
        <div className='relative z-10'>
            <button 
                ...
                className={`
                    bg-slate-950 text-sm sm:text-lg h-8 w-28 sm:w-56 sm:h-10 px-1.5 sm:px-3.5 sm:py-2 
                    rounded-lg border-white/15 border justify-between items-center gap-1 inline-flex
                `}>
                ...
                <img src="/Frame.png" className='w-3'/>
            </button>

            {isOpen && (
                    <div className={`absolute mt-1 sm:mt-2 w-28 sm:w-56 bg-slate-950  shadow-lg text-center rounded-lg border-white/15 border`}>
                        {Ranks.map((rank) => (
                            <div
                                onClick={() => handleClick(rank.rank, rank.name, rank.icon)}
                                className={`text-sm sm:text-lg flex items-center p-2 hover:bg-slate-800 cursor-pointer ${rank.rank == "" ? "rounded-t" : rank.rank == "HERALD" ? "rounded-b" : null}`}
                            >
