...
export default function Rank() {
    ...
    return(
        <div className='relative z-10'>
            <button onClick={() => setIsOpen(!isOpen)} className={`${router.asPath.includes('/hero/') ? 'bg-slate-900' : 'bg-slate-950'} text-sm sm:text-lg h-8 w-28 sm:w-44 sm:h-10 px-1.5 sm:px-3.5 sm:py-2 rounded-lg border border-slate-700 justify-between items-center gap-1 inline-flex`}>
            ...
            {isOpen && (
                    <div className={`absolute mt-1 sm:mt-2 w-28 sm:w-44 ${router.asPath.includes('/hero/') ? 'bg-slate-900' : 'bg-slate-950'}  shadow-lg text-center rounded-lg border border-slate-700`}>
                        {Ranks.map((rank) => (
                            <div
