...
cur.execute("TRUNCATE TABLE abilities")
...
for ability in abilityData:
    if ability['isTalent'] is not None and ability['stat'] is not None:
        ...
        if abilityName in abilities_json and abilityName != 'Empty':
...
def getQuery(hero_id):
    ...
    for role in roles:
        for rank in ranks:
            ...
            """
            queries.append(role_rank_query)
...
for hero_id in hero_ids:
    ...
    for rank in ranks:
        for role in roles:
            ...
            abilityMaxLevel = data['data'][objName]['abilityMaxLevel']
            abilityMinLevel = data['data'][objName]['abilityMinLevel']
            ...
            talents = data['data'][objName]['talent']
            
            if abilityMaxLevel and abilityMinLevel:
