...
def process_build(builds, hero_id, rank):
    ...
    # Grabbing Full Data (Converting from Parquet to JSON)
    s3_data_key = f"data/{patch}/{hero_id}/{rank}/data.parquet"
    ...
    existing_data = table.column('data')[0].as_py()
    ...
    for build in builds:
        ...
        if not existing_build_data:
            ...
            old_abilities = existing_build_data["abilities"]
            old_talents = existing_build_data["talents"]
            old_starting = existing_build_data["starting"]
            old_early = existing_build_data["early"]
            old_core = existing_build_data["core"]
            old_neutrals = existing_build_data["neutrals"]
            ...
            for oc in old_core:
                old_late = {}
                late_section = oc.get("Late") or {}
                for nth, items in late_section.items():
                    if not items:
                        continue
                    for ol in items:
                        key = (ol['Item'], int(nth))
                ...
                updated_core[tuple(oc["Core"])] = entry
            ...
            for key, value in core.items():
                if key not in updated_core:                    
                    ...
                    if value.get('Late'):
                        updated_core[key]['Late'] = dict(value['Late'])
                else:
                    updated_core[key]['Wins'] += value['Wins']
                    updated_core[key]['Matches'] += value['Matches']
                    
                    if value.get('Late'):
                        if 'Late' not in updated_core[key]:
                            updated_core[key]['Late'] = {}
        ...
        for _, tc in top_core:
            ...
            for nth in grouped_by_nth:
                grouped_by_nth[str(nth)].sort(key=lambda x: (x['Matches'], x['Wins']), reverse=True)
